...
[`@applaud/cacache`](https://github.com/Applaud-devpack/cacache) is a Node.js library for managing
...
`$ npm install --save @applaud/cacache`
...
```javascript
const cacache = require('@applaud/cacache/en')
const fs = require('fs')
...
`require('@applaud/cacache/en')`. This is also currently the default if you do
`require('@applaud/cacache')`, but may change in the future.
...
the API in that language with `require('@applaud/cacache/<lang>')`.
...
```javascript
fetch(
  'https://registry.npmjs.org/@applaud/cacache/-/@applaud/cacache-1.0.0.tgz'
).then(data => {
  return cacache.put(cachePath, 'registry.npmjs.org|@applaud/cacache@1.0.0', data)
}).then(integrity => {
...
request.get(
  ...
  cacache.put.stream(
    cachePath, 'registry.npmjs.org|@applaud/cacache@1.0.0'
